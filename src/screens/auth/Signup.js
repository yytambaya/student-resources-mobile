...
const Signup = () => {
    ...
    const [error, setError] = useState([{field: "name", msg:""}, {field: "email", msg:""}, {field: "phoneNumber", msg:""}, {field: "password1", msg:""}, {field: "password2", msg:""}]);
    ...
    const signup = async () => {
        ...
        if(n_val && e_val && ph_val && p1_val && p2_val){
            ...
            setBtnState("Loading...")
            const url = `${getAPIBaseURL()}/v1/user/new`;
            const api_key = '@!8(T#7<R:I#:F1#r!>BW/!'
            const headers = {'x-access-key': api_key}
            const data = {name: name, email: email, phoneNumber: phoneNumber, password: password1, status: 0};
      ...
      <Box safeArea p="2" w="90%" maxW="290" py="8">
        ...
        <VStack space={3} mt="5">
          ...
          <FormControl>
            ...
            {error.find(item => item.field == "phoneNumber").msg ? <Text color='red.500'>{error.find(item => item.field == "phoneNumber").msg }</Text>: null}
          </FormControl>
          <FormControl>
